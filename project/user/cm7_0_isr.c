...
void uart2_isr (void)
{
    if(uart_isr_mask(UART_2))            // 串口2接收中断
    {
       ...
       // upflow302_receive_callback();    // UP FLOW 302 光流回调（vendor 库函数，按库内 UART_INDEX 走 UART_2）：与 main.c 的 Up_Flow_302_Init 同步开关
...
void uart4_isr (void)
{
    if(uart_isr_mask(UART_4))            // 串口4接收中断
    {
        ...
        // uart_receiver_handler();                                                            // 串口接收机回调函数（当前未使用遥控器，先注释）
        uart_control_callback();                                                                // 无刷驱动速度回传解析函数
...
void uart6_isr (void)
{
    if(uart_isr_mask(UART_6))            // 串口6接收中断
    {
        ...
        upflow302_receive_callback();    // UP FLOW 302 光流回调（vendor 库函数，按库内 UART_INDEX 走 UART_6）
